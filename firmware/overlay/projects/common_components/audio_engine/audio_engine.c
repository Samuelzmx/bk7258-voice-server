...
static user_audio_vad_flag_func s_audio_vad_flag_update = NULL;
...
void audio_register_vad_flag_update_func(user_audio_vad_flag_func func)
{
    if (s_audio_vad_flag_update)
	{
		AUDE_LOGW("%s cb is already reg !!! %p %p\n", __func__, s_audio_vad_flag_update, func);
	}
	s_audio_vad_flag_update = func;
}
...
static bk_err_t audio_vad_flag_update_notify(unsigned char flag)
{
    if (s_audio_vad_flag_update)
    {
        return s_audio_vad_flag_update(flag);
    }
...
bk_err_t audio_turn_on(void)
{
    ...
    aud_intf_drv_setup.aud_intf_update_vad_flag = audio_vad_flag_update_notify;
