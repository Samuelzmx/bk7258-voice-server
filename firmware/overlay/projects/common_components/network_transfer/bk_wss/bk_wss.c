		...
		if (ret == BK_OK)
		{
			switch (msg.event)
			{
				...
				case WSS_EVT_SERVER_SESSION_UPDATED:
				{
					...
					LOGI("updated from server, waiting for greeting playback or timeout before recording\n");
					if (rtc) {
						rtc->response_audio_done_pending = 1;
						rtc->response_audio_started = 0;
						rtc->response_audio_idle_checks = 0;
					}
					break;
				}
				case WSS_EVT_SERVER_RSP_AUDIO_DONE:
				{
					rtc_session *rtc = __get_beken_rtc();

					if (!rtc || !g_connected_flag || rtc->disconnecting_state) {
						break;
					}
					...
					if (rtc->playing_state) {
						bk_wss_state_event(WSS_EVENT_PLAYING_END, NULL);
					}
				...
				case WSS_EVT_AUDIO_BUF_COMMIT:
					...
					break;
				case WSS_EVT_AUDIO_BUF_APPEND:
					LOGI("audio buf append\n");
					rtc_websocket_send_text(__get_beken_rtc(), (void *)(&dialog_info), BEKEN_RTC_INPUT_AUDIO_BUFFER_APPEND);
					break;
...
#if CONFIG_BK_WSS_TRANS_NOPSRAM
				if (session->response_audio_done_pending) {
					if (handled_packet) {
						session->response_audio_idle_checks = 0;
					} else if (((!session->response_audio_started) || session->playing_state)
							   && session->response_audio_idle_checks < 0xFFFF) {
						session->response_audio_idle_checks++;
					}

					if (!session->response_audio_started && session->response_audio_idle_checks >= 12) {
						LOGW("no startup greeting audio received, starting recording fallback\n");
						session->response_audio_done_pending = 0;
						session->response_audio_started = 0;
						session->response_audio_idle_checks = 0;
						websocket_event_send_msg(WSS_EVT_SERVER_RSP_AUDIO_DONE, 0);
					} else if (session->response_audio_started && session->playing_state
							   && session->response_audio_idle_checks >= 8) {
						LOGW("response.audio.done fallback fired after playback idle\n");
						session->response_audio_done_pending = 0;
						session->response_audio_started = 0;
						session->response_audio_idle_checks = 0;
						websocket_event_send_msg(WSS_EVT_SERVER_RSP_AUDIO_DONE, 0);
...
#ifdef DEBUG_CRC
			if ((HEAD_FLAGS_CRC & head.flags) && head.crc != 0)
			{
...
int rtc_websocket_audio_send_data(uint8_t *data_ptr, size_t data_len)
{
	...
	if (ret >= 0) {
		rtc_session->recording_bytes_sent += data_len;
	}
...
void rtc_get_dialog_info(aud_intf_voc_setup_t *aud_intf_voc_setup, char *encoder_name, char *decoder_name)
{
    ...
    rtc_fill_dialog_info(&dialog_info, devId_mac_str, "", encoder_name, aud_intf_voc_setup->aud_codec_setup_input.adc_samp_rate,
                         decoder_name, aud_intf_voc_setup->aud_codec_setup_input.dac_samp_rate, 0, "BKR1");
}
