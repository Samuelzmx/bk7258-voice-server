...
#define RESPONSE_AUDIO_IDLE_CHECKS_BEFORE_RECORD 8
...
static int bk_wss_playback_complete(void *param)
{
    rtc_session *rtc = __get_beken_rtc();

    if (rtc == NULL || !g_connected_flag || rtc->disconnecting_state) {
        return BK_OK;
    }
    websocket_event_send_msg(WSS_EVT_SERVER_RSP_AUDIO_DONE, 0);

    return BK_OK;
}

static int bk_wss_vad_flag_update(unsigned char flag)
{
    rtc_session *rtc = __get_beken_rtc();
    ...
    if (!rtc->recording_state || rtc->commit_pending) {
        return BK_OK;
    }

    if (flag == 1) {
        LOGI("VAD speech start\n");
    } else if ((flag == 2 || flag == 3) && rtc->recording_bytes_sent > 0) {
        LOGI("VAD speech end, committing %u bytes\n", rtc->recording_bytes_sent);
        websocket_event_send_msg(WSS_EVT_RECORDING_END, 0);
    }
...
int32_t bk_wss_state_event(uint32_t event, void *param)
{
	...
	switch (event) {
		case WSS_EVENT_RECORDING_START:
			...
			if (wss_record_work(rtc) == 0) {
				rtc->recording_bytes_sent = 0;
				rtc->commit_pending = 0;
				wss_record_start(rtc);
		...
		case WSS_EVENT_RECORDING_END:
			...
			if (rtc->recording_bytes_sent > 0 && !rtc->commit_pending) {
				rtc->commit_pending = 1;
		...
		case WSS_EVENT_PLAYING_START:
			...
			if (rtc) {
				...
				rtc->response_audio_done_pending = 0;
				rtc->response_audio_idle_checks = 0;
				rtc->recording_bytes_sent = 0;
				rtc->commit_pending = 0;
				wss_play_start(rtc);
			}
...
void bk_websocket_msg_handle(char *json_text, unsigned int size)
{
    ...
    if (json_text == NULL || size == 0) {
        LOGE("Error: empty websocket text frame.\n");
        return;
    }

    json_copy = os_malloc(size + 1);
    if (json_copy == NULL) {
        LOGE("Error: failed to allocate websocket text buffer, size:%u\n", size);
        return;
    }

    os_memcpy(json_copy, json_text, size);
    json_copy[size] = '\0';
	...
	if (strcmp(type->valuestring, "hello_response") == 0) {
		...
		if (ret == 200) {
			...
			LOGI("hello_response ok, agent joined state restored for hands-free mode\n");
        ...
        __get_beken_rtc()->commit_pending = 0;
        __get_beken_rtc()->recording_bytes_sent = 0;
        ...
        LOGI("response.created, waiting for first audio frame before starting playback\n");
        text_info_t info = {};
        ...
        if (__get_beken_rtc()) {
            if (__get_beken_rtc()->playing_state) {
                __get_beken_rtc()->response_audio_done_pending = 1;
                __get_beken_rtc()->response_audio_started = 1;
                __get_beken_rtc()->response_audio_idle_checks = 0;
                LOGI("response.audio.done marked pending; waiting for playback path to go idle\n");
            } else if (!__get_beken_rtc()->recording_state
                && !__get_beken_rtc()->commit_pending) {
                websocket_event_send_msg(WSS_EVT_SERVER_RSP_AUDIO_DONE, 0);
            }
        }
...
void bk_websocket_event_handler(void* event_handler_arg, char *event_base, int32_t event_id, void* event_data)
{
	...
	case WEBSOCKET_EVENT_DATA:
			...
			if (data->op_code == WS_TRANSPORT_OPCODES_BINARY) {
				rtc_session *rtc = __get_beken_rtc();
				if (!rtc) {
					LOGW("dropping binary audio because rtc session is null\n");
					break;
				}
#if CONFIG_BK_WSS_TRANS_NOPSRAM
				if (!rtc->playing_state) {
					LOGI("first response audio frame received, starting playback\n");
					bk_wss_state_event(WSS_EVENT_PLAYING_START, NULL);
				}
				rtc->response_audio_started = 1;
				rtc->response_audio_idle_checks = 0;
#endif
				rtc_websocket_audio_receive_data_general(rtc, (uint8_t *)data->data_ptr, data->data_len);
			}
...
void beken_wss_main(void)
{
    bk_err_t ret = BK_OK;
    memory_free_show();
    audio_register_play_finish_func(bk_wss_playback_complete);
    audio_register_vad_flag_update_func(bk_wss_vad_flag_update);
