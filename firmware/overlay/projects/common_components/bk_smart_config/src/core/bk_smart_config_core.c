...
#define WSS_DEV_WIFI_NO_AP_MAX_RETRY  (3)

static bool s_dev_wifi_fallback_attempted = false;
static bool s_dev_wifi_fallback_active = false;
static bool s_auto_boarding_started = false;
static uint16_t s_wifi_no_ap_failures = 0;

static int bk_sconf_try_dev_wifi_fallback(void)
{
    if (s_dev_wifi_fallback_attempted) {
        return BK_FAIL;
    }

    s_dev_wifi_fallback_attempted = true;
    s_dev_wifi_fallback_active = true;
    s_wifi_no_ap_failures = 0;

    BK_LOGW(TAG, "saved Wi-Fi unavailable, trying development fallback SSID:%s\r\n", WSS_DEV_WIFI_SSID);
    network_reconnect_stop_timeout_check();
    network_reconnect_start_timeout_check(50);
    app_event_send_msg(APP_EVT_RECONNECT_NETWORK, 0);
    network_disc_evt_posted = 0;

    return demo_sta_app_init(WSS_DEV_WIFI_SSID, WSS_DEV_WIFI_PASSWORD);
}

static void bk_sconf_start_auto_boarding_once(void)
{
    if (s_auto_boarding_started) {
        return;
    }

    s_auto_boarding_started = true;
    s_dev_wifi_fallback_active = false;
    BK_LOGW(TAG, "development Wi-Fi fallback failed, clearing saved Wi-Fi info and entering boarding mode\r\n");
...
#if CONFIG_BK_WSS_TRANS_NOPSRAM
                s_wifi_no_ap_failures = 0;
                s_auto_boarding_started = false;
                if (s_dev_wifi_fallback_active) {
                    bk_wifi_sta_get_config(&sta_config);
                    demo_save_network_auto_restart_info(got_ip->netif_if, &sta_config);
                    s_dev_wifi_fallback_active = false;
                }
...
#if CONFIG_BK_WSS_TRANS_NOPSRAM
            if (!smart_config_running &&
                (sta_disconnected->disconnect_reason == WIFI_REASON_NO_AP_FOUND)) {
                if (s_wifi_no_ap_failures < 0xFFFF) {
                    s_wifi_no_ap_failures++;
                }

                BK_LOGW(TAG, "no AP found, fail_count:%u fallback_attempted:%d fallback_active:%d\r\n",
                        s_wifi_no_ap_failures, s_dev_wifi_fallback_attempted, s_dev_wifi_fallback_active);

                if (!s_dev_wifi_fallback_attempted) {
                    if (bk_sconf_try_dev_wifi_fallback() == BK_OK) {
                        break;
                    }
                } else if (s_dev_wifi_fallback_active &&
                           (s_wifi_no_ap_failures >= WSS_DEV_WIFI_NO_AP_MAX_RETRY)) {
                    bk_sconf_start_auto_boarding_once();
                    break;
                }
            }
#endif
